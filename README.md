...
**現在の実装モジュール: `cavsim` v0.2.1** (レーザー共振器シミュレータ)
...
```bash
git clone https://github.com/ya009302-star/laser-os.git
cd laser-os
pip install -e .
cavsim        # GUI起動
```
...
コア物理・解析機能 58 件。解析解との照合 (平凹共振器ウエスト、g1g2安定性、
ブリュースター縮約長比、非点収差補償角、ミスアライメント感度、分散記帳、
出力ビームM²復元など) を含む。
...
- ミスアライメント感度 (ミラー傾き → 結晶/端面鏡上のビーム移動量, µm/mrad)
- 2D スタビリティマップ (間隔 2 つの同時掃引、クリックで設定)
- 安定許容差レポート (各間隔があと何 mm で不安定化するか)
- 出力ビーム伝搬 (OC の外側の w(z)) と z–w フィット (M² 推定)
